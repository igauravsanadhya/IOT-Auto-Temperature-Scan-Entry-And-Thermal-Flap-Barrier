...
m1 = LED(18)
m2 = LED(17)
button = Button(pin=23,pull_up=True, active_state=None, bounce_time=1, hold_time=1, hold_repeat=True, pin_factory=None)
def counter():
    global p_count
    global temperature
    temperature = sensor.get_object_1()
    print("Person Temperature :", temperature )
    p_count = p_count +1
    print("counter: ",p_count)
def ir_released():
        print("NEXT!!")
...
def led_trig():
    ...
    while button.is_pressed:
        ...
        if (allowed <=19) :
            if mask_on and (temperature < 35.0):
                m1.on()
                m2.off()
                time.sleep(2)
                m1.off()
        ...
        if key == ord("q"):
            break
button.when_held = led_trig
button.when_pressed = counter
button.when_released = ir_released
# while True:
    # frame1 = vs.read()
